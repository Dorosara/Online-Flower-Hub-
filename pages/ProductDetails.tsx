...
export const ProductDetails = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link to="/shop" className="inline-flex items-center text-gray-600 hover:text-primary-600 mb-8 transition-colors font-medium">
        <ArrowLeft className="h-4 w-4 mr-1" /> Back to Shop
      </Link>
      
      <div className="bg-white/85 backdrop-blur-md rounded-2xl shadow-xl border border-pink-100 p-6 md:p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          {/* Image */}
          <div className="relative rounded-xl overflow-hidden bg-gray-100 aspect-w-4 aspect-h-3 shadow-md">
            <img 
              src={product.image} 
              alt={product.name} 
          ...
          <div className="flex flex-col h-full justify-center">
            ...
            <div className="border-t border-b border-gray-200/60 py-6 mb-8 space-y-4">
              <div className="flex items-center text-sm text-gray-600">
                <Check className="h-4 w-4 text-green-500 mr-2" /> In Stock: {product.stock > 0 ? 'Ready to ship' : 'Out of stock'}
              </div>
              <div className="flex items-center text-sm text-gray-600">
                <Truck className="h-4 w-4 text-primary-500 mr-2" /> Free delivery on orders over ₹100
              </div>
            </div>
            ...
            {product.stock > 0 ? (
              <div className="space-y-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center border border-gray-300 rounded-md bg-white/50">
                    <button 
                      ...
                      className="p-2 hover:bg-white text-gray-600"
                    >
                ...
                <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
                  <Button onClick={handleAddToCart} size="lg" className="flex-1 shadow-md">
                    Add to Cart
                  </Button>
                  <Button onClick={() => { handleAddToCart(); navigate('/cart'); }} variant="secondary" size="lg" className="flex-1 shadow-md">
                    Buy Now
                  </Button>
                </div>
              </div>
